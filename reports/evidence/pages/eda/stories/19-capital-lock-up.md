...
select
    round(avg(avg_days_of_supply), 0) as current_days,
    90 as target_days,
    round((avg(avg_days_of_supply) - 90) * avg(avg_sell_through_rate) * (select sum(total_revenue) from datathon_warehouse.mart_product_lifetime_performance where lifecycle_stage = 'active') / avg(avg_days_of_supply), 0) as implied_freed_revenue
from datathon_warehouse.mart_monthly_inventory_snapshot
```
...
a substantial amount of working capital would be freed for reinvestment in marketing, product development, or debt reduction.
</Alert>
