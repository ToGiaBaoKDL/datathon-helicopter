...
<Alert status="info">
The retention curve reveals a brutal cliff: most customers who do not return within the first month never return at all.
After month 1, the curve flattens — survivors are essentially retained for life.
The battle is won or lost in the first 30 days.
</Alert>
...
Dark cells = better retention. Most cohorts fade to near-zero by month 3, confirming the cliff pattern.
</Alert>
...
<Heatmap
    ...
    subtitle="Each row = one customer cohort. Each column = months since first order."
    valueFmt="pct2"
/>
...
<b><Value data={channel_ratio} column=ratio fmt=0.0/>×</b> the rate of <Value data={worst_channel} column=worst_channel/> (<Value data={worst_channel} column=worst_m1 fmt=pct2/>). 
...
See also <a href="/01-stories/customer/03-unit-economics-map">Story 03: Unit Economics Map</a> for channel-level LTV and retention analysis.
</Alert>
